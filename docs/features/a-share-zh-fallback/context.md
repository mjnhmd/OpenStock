...
### 可用性修复（第三轮）
用户反馈"没有美股股票热力图"。用真实浏览器（Playwright + 本机 Chrome +
Clash 代理 7890）复现后确认：**代码没问题，是默认市场变成了 A 股**，
而且 `DonatePopup` 在加载 3 秒后自动弹模态框盖住整页，右上角的切换开关
被遮挡，用户很难发现。

处理：
- 切换控件增加「市场」前缀文字，`border-gray-600` + 选中态加粗，更显眼。
- 新增 `MarketContextBar`，在仪表盘顶部显示「当前市场 [A股/美股] +
  该市场包含哪些面板 + 在右上角切换市场」。
- 未改默认市场（仍为 A 股），因为用户第一诉求是 A 股。

浏览器实测证据（headless Chrome + 代理）：
| 状态 | 面板标题 | TradingView iframe |
|---|---|---|
| 默认（无 cookie） | 行业板块热力图 / 涨幅榜 / 概念板块热力图 | 0 |
| 点击「美股」 | 市场概览 / 股票热力图 | 4（含 `embed-widget/stock-heatmap?dataSource=SPX500`） |
| 点击「A股」 | 行业/概念热力图 | 0 |

### 个股级热力图（第四轮）
用户要求 A 股也要有跟美股一样的**个股**热力图（参考
quote.eastmoney.com/stockhotmap）。

- 数据：东财 `clist` 全市场，`pz` 硬上限 100 行 → 56 页，并发 8，实测 **598ms**
  返回 5560 行，含 `f20` 总市值 / `f21` 流通市值 / `f100` 行业。
- 布局：参考 `wenyuanw/a-share-heatmap`（MIT）的**二分树图**思路自行实现
  （`lib/market-data/heatmap-layout.ts`）：按累计市值取二分点，始终切长边。
- 渲染：Canvas（不是 DOM），支持 5000+ 方块；`devicePixelRatio` 适配、
  ResizeObserver 自适应、悬浮 tooltip、点击进入个股页。
- 加权：**流通市值**（不是总市值），与参考项目一致。
- 分组：把 `银行Ⅱ / 白酒Ⅲ` 这类二级三级行业名归一成一级行业。
- 红涨绿跌。
- 板块级（行业/概念）热力图与涨幅榜**全部保留**，用户明确要求"都保留"。
